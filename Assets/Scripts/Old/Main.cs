...
public class Main : MonoBehaviour
{
	...
	{ 
		...
		if (Mathf.Clamp01 (_Level.transform.localScale.x) >= 1) { 
			...
			var destroyingParticle = GameObject.Instantiate (_LevelTextValue as GameObject, 
                new Vector3 (_LevelTextValue.transform.position.x, _LevelTextValue.transform.position.y, 
                _LevelTextValue.transform.position.z - 1), transform.rotation) as GameObject;
	...
	{   //replace the empty tiles with the ones above
		...
		for (int x = 0; x <= cells.GetUpperBound(0); x++) {
			for (int y = 0; y <= cells.GetUpperBound(1); y++) {
				...
				HOTween.To (cells [x, y].transform, .1f, parms);
			}
		}
	...
	void DoShapeEffect ()
	{
		...
		foreach (GameObject row in _currentParticleEffets)
			Destroy (row);
		for (int i = 0; i <= 2; i++)
			_currentParticleEffets.Add (GameObject.Instantiate (_particleEffect, 
                new Vector3 (Random.Range (0, _arrayOfShapes.GetUpperBound (0) + 1), 
                             Random.Range (0, _arrayOfShapes.GetUpperBound (1) + 1), -1), 
                new Quaternion (0, 0, Random.Range (0, 1000f), 100)) as GameObject);
        */
	}
	...
	{   	
		for (int r = 1; r <= _theFreezeNumberOfGems; r++) {
			...
			if (firstTile != null) {	
				firstTile.tag = "Untagged 1";
			} 
			if (secondTile != null) {
				secondTile.tag = "Untagged 2";
			} 
			if (thirdTile != null) {
				thirdTile.tag = "Untagged 3";
			} 
			if (fourthTile != null) {	
				fourthTile.tag = "Untagged 4";
			}  
